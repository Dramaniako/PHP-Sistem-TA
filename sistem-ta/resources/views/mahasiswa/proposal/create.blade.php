<x-app-layout title="Ajukan Judul Proposal">
    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8"> {{-- Lebar saya ubah ke max-w-5xl agar grid muat --}}
            ...
            @if(isset($proposal) && $proposal->status === 'ditolak')
                <div class="bg-red-50 border-l-4 border-red-500 text-red-700 p-4 mb-6 shadow-sm rounded-r-lg" role="alert">
                    <div class="flex items-center mb-2">
                        <i class="fas fa-exclamation-circle mr-2"></i>
                        <p class="font-bold text-lg">Proposal Perlu Diperbaiki</p>
                    </div>
                    <p class="text-sm">Alasan Penolakan: <span class="font-semibold text-red-800">{{ $proposal->catatan_koordinator ?? 'Silakan perbaiki dokumen atau judul sesuai arahan koordinator.' }}</span></p>
                </div>
            @endif
                ...
                <div class="p-8 text-gray-900">
                    ...
                    <form action="{{ route('mahasiswa.proposal.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="judul" value="{{ old('judul', $proposal->judul ?? '') }}" required 
                        ...
                        <div class="mb-8">
                            ...
                            <textarea name="deskripsi" rows="5" required 
                                ...
                                placeholder="Jelaskan secara singkat latar belakang dan tujuan penelitian...">{{old('deskripsi', $proposal->deskripsi ?? '')}}</textarea>
                            @error('deskripsi')
                                <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="border-2 border-dashed border-gray-300 bg-gray-50 rounded-xl p-6 hover:bg-blue-50 hover:border-blue-300 transition group">
                                    ...
                                    <input type="file" name="file_proposal" accept=".pdf" {{ isset($proposal) ? '' : 'required' }} 
                                    ...
                                    @if(isset($proposal->file_proposal))
                                        <p class="text-[10px] text-green-600 mt-1 font-semibold">
                                            <i class="fas fa-check-circle"></i> File proposal sudah terunggah. Unggah baru untuk mengganti.
                                        </p>
                                    @endif
                                    @error('file_proposal')
                                ...
                                <div class="border-2 border-dashed border-gray-300 bg-gray-50 rounded-xl p-6 hover:bg-yellow-50 hover:border-yellow-300 transition group">
                                    <div class="flex items-center gap-3 mb-3">
                                        ...
                                        <div>
                                            <h4 class="font-bold text-gray-700 text-sm">Kartu Hasil Studi (KHS) dan Sertifikat WTA</h4>
                                            <p class="text-[10px] text-gray-400">Wajib, Format PDF (Max 5MB)</p>
                                        </div>
                                    </div>
                                    <input type="file" name="file_khs" accept=".pdf" {{ isset($proposal) ? '' : 'required' }} 
                                        class="block w-full text-xs text-gray-500 
                                        file:mr-4 file:py-2 file:px-4 
                                        file:rounded-full file:border-0 
                                        file:text-xs file:font-semibold 
                                    ...
                                    @if(isset($proposal->file_khs))
                                        <p class="text-[10px] text-green-600 mt-1 font-semibold">
                                            <i class="fas fa-check-circle"></i> File sudah terunggah.
                                        </p>
                                    @endif
                                    @error('file_khs')
                                        <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                        ...
                        <div class="flex items-center justify-end pt-6 border-t border-gray-100 mt-6">
                            ...
                            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-xl shadow-lg transform transition hover:-translate-y-0.5 hover:shadow-blue-200 flex items-center gap-2">
                                <i class="fas fa-paper-plane"></i> {{ isset($proposal) ? 'Simpan Perubahan' : 'Kirim Pengajuan' }}
                            </button>
                        </div>
